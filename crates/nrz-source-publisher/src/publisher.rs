...
use nrz_api::{
    ...
    MultipartCompleteRequestBodyPart as CliMultipartCompletePart,
    PrepareUpload200Response as CliPrepareUploadResponse,
    PrepareUpload200ResponseRequiredComplete as CliPrepareUploadRequiredComplete,
    PrepareUploadRequestBody as CliPrepareUploadRequest,
    PrepareUploadRequestBodyLogicalManifestSummary as CliLogicalManifestSummary,
    PrepareUploadRequestBodyMultipart as CliPrepareMultipart,
    PrepareUploadRequestBodyMultipartPart as CliPrepareMultipartPart,
    PrepareUploadRequestBodySourceUploadRecovery as CliPrepareUploadSourceUploadRecovery,
...
use nrz_contract::RuntimeArtifactGraphV2Wire;
    ...
    let multipart = request
        ...
        .map(|multipart| {
            let parts = multipart
                ...
                .map(|part| {
                    Ok(CliPrepareMultipartPart {
                        part_number: i64::from(part.part_number),
                        size_bytes: i64::try_from(part.size_bytes).map_err(|_| {
                            invalid_source("multipart part size exceeds wire range")
                        })?,
            ...
            Ok::<CliPrepareMultipart, SourcePublicationError>(CliPrepareMultipart {
                part_count: i64::try_from(parts.len())
                    .map_err(|_| invalid_source("multipart part count exceeds wire range"))?,
                part_size_bytes: i64::try_from(multipart.part_size_bytes)
                    .map_err(|_| invalid_source("multipart part size exceeds wire range"))?,
                parts,
            })
        })
        .transpose()?;
        ...
        (None, Some(target)) => {
            ...
            for chunk in &target.chunks {
                ...
                let offset = u64::from(part_number.saturating_sub(1))
                    .checked_mul(signed_size(target.chunk_size, "multipart chunk size")?)
    ...
    retry_control_plane(
        ...
        || async {
            match request
                ...
                .await?
            {
                CliUploadCompleteResponse::Object(_)
                | CliUploadCompleteResponse::Object2(_)
                | CliUploadCompleteResponse::Object3(_)
                | CliUploadCompleteResponse::Object6(_) => Ok(()),
                CliUploadCompleteResponse::Object5(_) => Err(
                    StructuredControlPlaneError::retryable("SOURCE_UPLOAD_INCOMPLETE"),
                ),
                CliUploadCompleteResponse::Object4(expired) => {
                    Err(SourcePublicationError::InvalidResponse(format!(
                        "source upload window expired at {}",
                        expired.expired_at
                    )))
                }
            }
    ...
    O: PublicationObserver,
{
    ...
    loop {
        let status = request.transport.deployment_status(request.deployment_id);
        let status = match tokio::time::timeout(
            DURABLE_READBACK_BUDGET.saturating_sub(started.elapsed()),
            status,
        )
        .await
        .map_err(|_| {
            SourcePublicationError::Deadline(
                "durable readback request exceeded its deadline".into(),
            )
        })? {
            Ok(status) => status,
...
async fn report_upload_failed<T, O>(
    ...
    O: PublicationObserver,
{
    let error_code = SOURCE_UPLOAD_PUT_FAILED.to_string();
    let error_log = bounded_failure_log(error);
    let source_artifact_id = prepared.source_artifact_id.clone();
    ...
    Fut: Future<Output = Result<T, SourcePublicationError>>,
{
    ...
    loop {
        ...
        let result = tokio::time::timeout(remaining, call()).await.map_err(|_| {
            SourcePublicationError::Deadline(format!("{operation} exceeded {budget:?}"))
        })?;
        match result {

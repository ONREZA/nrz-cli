...
use nrz_api::Prepare200ResponseUpload as RuntimeDependencyUpload;
pub use nrz_api::{
    Complete200Response as RuntimePublicationCompleteResponse,
    Prepare200Response as RuntimePublicationPrepareResponse,
    PrepareRequestBody as RuntimePublicationPrepareRequest,
    PrepareRequestBody as RuntimePublicationCompleteRequest,
};
    ...
    O: PublicationObserver,
{
    ...
    Ok(PublishedRuntimeArtifacts {
        ...
        runtime_artifact_graph: request.expected_graph.wire().clone(),
    })
}

fn publication_request<T, O>(
    request: &RuntimeArtifactPublicationRequest<'_, T, O>,
) -> Result<RuntimePublicationPrepareRequest, SourcePublicationError> {
    Ok(RuntimePublicationPrepareRequest {
        ...
        dependencies: request
            ...
            .iter()
            .map(|dependency| {
                Ok(nrz_api::PrepareRequestBodyDependency {
                    layer_name: dependency.layer_name.to_string(),
                    mount_point: dependency.mount_point.to_string(),
                    manifest: serde_json::from_value(
                        serde_json::to_value(dependency.manifest.wire()).map_err(|_| {
                            invalid_response("cannot serialize verified dependency manifest")
                        })?,
                    )
                    .map_err(|_| invalid_response("dependency manifest does not match OpenAPI"))?,
                })
            })
            .collect::<Result<_, SourcePublicationError>>()?,
    })
}
...
fn verify_response_graph(
    ...
    actual_digest: &str,
    actual: &nrz_api::Status200ResponseRuntimeArtifactGraph,
) -> Result<(), SourcePublicationError> {
    if actual_deployment_id != expected_deployment_id
        || actual_digest != expected.graph_digest()
        || serde_json::to_value(actual)
            .map_err(|_| invalid_response("cannot serialize returned graph"))?
            != serde_json::to_value(expected.wire())
                .map_err(|_| invalid_response("cannot serialize verified graph"))?
    {
        return Err(invalid_response(
